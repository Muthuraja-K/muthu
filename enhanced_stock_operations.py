...
def get_yahoo_realtime_data(ticker: str) -> Dict[str, Any]:
    """Get real-time data from Yahoo Finance including after-hours data"""
    try:
        ticker_obj = yf.Ticker(ticker)
        
        # Get real-time price with after-hours data
        current_price = None
        today_change = None
        
        try:
            # Get 1-minute interval data for today (includes after-hours)
            # This provides real-time data including pre-market and after-hours trading
            live_data = ticker_obj.history(period='1d', interval='1m', prepost=True)
            if not live_data.empty:
                current_price = live_data.iloc[-1]['Close']
                
                # Calculate today's change based on previous close (consistent with cached data)
                try:
                    info = ticker_obj.info
                    ...
                    logging.warning(f"Could not calculate today's change for {ticker}: {e}")
        except Exception as e:
            logging.warning(f"Could not get live data for {ticker}: {e}")
        
        # Fallback to regular market data if live data fails
        if not current_price:
            try:
                info = ticker_obj.info
                ...
                logging.warning(f"Could not get fallback data for {ticker}: {e}")
